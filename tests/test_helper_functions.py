...
class TestValidateUri:
    """Test the validate_uri helper function."""
    
    def test_valid_http_uri(self):
        """Test that valid HTTP URIs pass validation."""
        validate_uri("http://example.org/test")  # Should not raise
        validate_uri("https://example.org/test")  # Should not raise
    
    def test_invalid_identifiers_raise_error(self):
        """Test that invalid RDF identifiers raise ToolError."""
        invalid_identifiers = [
            "",  # Empty string
            "   ",  # Whitespace only
